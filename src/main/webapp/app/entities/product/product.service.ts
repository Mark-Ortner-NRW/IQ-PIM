...
export class ProductService {
  ...
  protected convertDateFromClient(product: IProduct): IProduct {
    const copy: IProduct = Object.assign({}, product, {
      ...
      modifiedAt: product.modifiedAt && product.modifiedAt.isValid() ? product.modifiedAt.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((product: IProduct) => {
        ...
        product.modifiedAt = product.modifiedAt ? moment(product.modifiedAt) : undefined;
      });
    }
    return res;
  }
